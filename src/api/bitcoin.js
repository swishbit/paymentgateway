...
const createTransaction = transaction => {
  return new Promise((resolve, reject) => {
    ...
    const insight = new explorers.Insight(bitcore.Networks.testnet)
    ...
    insight.getUnspentUtxos(transaction.fromaddress, function(error, utxos) {
      if (error) {
        ...
        if (utxos.length == 0) {
          ...
          return reject(
            "You don't have enough Satoshis to cover the miner fee."
          )
        }
        //get balance
        ...
        for (var i = 0; i < utxos.length; i++) {
          balance += unit
            .fromSatoshis(parseInt(utxos[i]["satoshis"]))
            .toSatoshis()
        }
...
const convert = ({ value, currency = "SEK" }) => {
  return new Promise((resolve, reject) => {
    resolve(value)
  })
}
